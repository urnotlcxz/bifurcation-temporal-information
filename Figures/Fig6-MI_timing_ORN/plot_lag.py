...
lags_ORN = np.load('./data/ORN_lag_2.npy')
lags_ORN_Ca = np.load('./data/NaKCa_lag_2.npy')
lags_NaK = np.load('./data/NaK_lag_3.npy')

lags_ORN_m = np.mean(lags_ORN, axis=1)
lags_ORN_err = scipy.stats.sem(lags_ORN, axis=1)
...
ORN_spearman = scipy.stats.spearmanr(mus, np.array(lags_ORN_m))
...
plt.plot(mus, (lags_ORN_m-lags_ORN_m[0])*dt1, lw=3, color='tab:blue')
...
plt.text(8.3,-1.1,r'$\rho$={0}, p={1}'.format(round(NaK_spearman[0],3), round(NaK_spearman[1],3)),
         fontsize=13, color='tab:green')
plt.text(8.3,-1.45,r'$\rho$={0}, p={1}'.format(round(ORN_spearman[0],3), round(ORN_spearman[1],3)),
