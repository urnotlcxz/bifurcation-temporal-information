...
class SimplifiedLIFModel():
    ...
    def __init__(self, dt=0.1, N=1000000, odor_tau=200, odor_mu=4.53, odor_sigma=0.3,
                 ...
                 k_rate=1./200, rate_sigma=0.5):
        """
        初始化LIF模型参数
        2019年 台湾LIF全仿真中：V_th=-45.0, V_reset=-70.0, V_rest=-55.0
        参数:
        dt: 时间步长(ms)
        N: 模拟步数
    ...
    def integrate(self):
        ...
        for i in range(1, self.N):
            ...
            # 更新适应电流——替代原文中的公式2和3
            d_adapt = -adaptation[i-1] / self.adaptation_tau  # 模拟钙清除过程
            adaptation[i] = adaptation[i-1] + d_adapt * self.dt  # 模拟钙积累效应
    ...
    def calc_rate(self,filt_type='Gaussian', filt_len=30000):
        """计算连续发放率"""
        # 高斯滤波平滑脉冲序列
        if filt_type == 'Gaussian':
            ...
            self.rate = np.convolve(filt, self.spikes, mode='full')[:self.N]
...
class NoAdaptiveLIFModel():
    ...
    def __init__(self, dt=0.1, N=1000000, odor_tau=200, odor_mu=4.53, odor_sigma=0.3,
                 tau_m=10.0, R=1.0, V_th=-45.0, V_reset=-60.0, V_rest=-55.0,
    ...
    def integrate(self):
        ...
        for i in range(1, self.N):
            # LIF模型方程
            dV = (-(V[i-1] - self.V_rest) + self.R * self.stim[i] ) / self.tau_m
            V[i] = V[i-1] + dV * self.dt
