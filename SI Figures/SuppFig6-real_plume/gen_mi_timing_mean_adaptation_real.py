...
This script generates temporal information heatmap using Na+K+Ca model when 
...
for mu in mus:
    ...
    for sigma in sigmas:
        ...
        bin_stim_Ca = np.zeros(len(testsignal))
        
        bin_stim_Ca[np.where(testsignal > 4.54)[0]] = 1
        ...
        if all(bin_rate_Ca == 0) or all(bin_rate_Ca == 1) or len(on_rate_Ca) < 5 or len(off_rate_Ca) < 5:
            ...
            stims_Ca_cut = testsignal[int(cutout/dt):]
